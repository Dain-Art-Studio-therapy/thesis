    ...
    if contact_sphere == []:
        ...
        if light_contact_sphere != []:
            if (length_vector(vector_from_to(point_epsilon, light.pt)) > 
                length_vector(vector_from_to(
                    light_contact_sphere[light_check][1], point_epsilon))): 
                    # light blocked by another sphere
                finish_red = finish_ambient.r
                finish_green = finish_ambient.g
                finish_blue = finish_ambient.b
            else:
...
def cast_all_rays(
    ...
    for y in range(height):
        for x in range(width):
            ...
            print >> output, ppm_red, ppm_green, ppm_blue # color in file color format
