...
def sphere_intersection_point(ray, sphere):
  ...
  c = vm.dot_vector(vm.difference_point(ray.pt, sphere.center), vm.difference_point(ray.pt, sphere.center)) - sphere.radius ** 2
  
  dis = b**2 -  4 * a * c
  if dis < 0:
    return None

  oneRoot = (((-1 * b) + math.sqrt(dis))) / (2* a)
  otherRoot = (((-1 * b) - math.sqrt(dis))) / (2 * a)
