# WARNING: IT'S VERY MESSY, I DIDN'T HAVE A LOT OF TIME TO CLEAN IT UP
...
def cast_ray(ray, sphere_list, ambient_color, point_light, eye_pos):
   ...
   if len(intersections) > 0:
      ...
      scaled_v1 = vector_math.scale_vector(v1, 0.001)
   ...
   for y in range(height):
      for x in range(width):
         ...
         if get_color is not data.Color(1.0, 1.0, 1.0):
            print int(cap_value(get_color.r * 255)),\
                  int(cap_value(get_color.g * 255)),\
                  int(cap_value(get_color.b * 255))
         else:
            print int(255), int(255), int(255)
