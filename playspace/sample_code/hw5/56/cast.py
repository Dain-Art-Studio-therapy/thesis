...
def light_model(nearest_intersect, sphere_list, ambient_light,
    ...
    specular = specular_component(nearest_intersect, light, eye_point, sphere_list)
...
def specular_component(nearest_intersect, light, eye_point, sphere_list):
    ...
    if specular_intensity > 0 and not light_obstructions(point, sphere_list, ray, light_vector):
...
def cast_all_rays(min_x, max_x, min_y, max_y, width, height,
    ...
    for h in range(height):
        ...
        for w in range(width):
            ...
            ray_caster.output_file(output, rgb)

            time2 = time.clock()
            time_one = time2 - time1
            total_time = time_one * total_pixels

            time_remaining = \
                estimate_time_remaining(time_one, total_time, pixel_count)

            sys.stdout.write('\r' + str(pixel_count) + '/' + str(total_pixels) +
...
def estimate_time_remaining(time_one, total_time, pixel_count):
    time_remaining = total_time - (time_one * pixel_count)
    #if time_remaining < 0.00000000001:
        #return 0.0
