import data
import math

def scale_vector(vector, scalar):
    return data.Vector(vector.x*scalar, vector.y*scalar, vector.z*scalar)


def dot_vector(vector1, vector2):
    return vector1.x*vector2.x + vector1.y*vector2.y + vector1.z*vector2.z


def length_vector(vector):
    return math.sqrt(vector.x**2 + vector.y**2 + vector.z**2)


def normalize_vector(vector):
    length = 1/length_vector(vector)
    pt1 = length * vector.x
    pt2 = length * vector.y
    pt3 = length * vector.z
    return data.Vector(pt1, pt2, pt3)

def difference_point(point1, point2):
    pt1 = point1.x - point2.x
    pt2 = point1.y - point2.y
    pt3 = point1.z - point2.z
    return data.Vector(pt1, pt2, pt3)

def difference_vector(vector1, vector2):
    pt1 = vector1.x - vector2.x
    pt2 = vector1.y - vector2.y
    pt3 = vector1.z - vector2.z
    return data.Vector(pt1, pt2, pt3)

def translate_point(point, vector):
    return data.Point(point.x + vector.x, point.y + vector.y, point.z + vector.z)


def vector_from_to(from_point, to_point):
    return data.Vector(to_point.x - from_point.x, to_point.y - from_point.y, to_point.z - from_point.z)
