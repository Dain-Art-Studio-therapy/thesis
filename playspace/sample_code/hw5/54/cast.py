...
# Global Var, sorry. Using it to get rid of magic number in cast_all_rays
maxColor = 255
...
def cast_all_rays(min_x, max_x, min_y, max_y, width, height, eye_point, 
   sphere_list, ambient, light, output):

   print_header(width,height, output)

   xIncrement = (max_x - min_x) / width
   yIncrement = (max_y - min_y) / height
   ...
   for j in range(int(height)):
      for i in range(int(width)):
         ...
         print >> output, min(int(maxColor * displayColor.r), 255) , 
         print >> output, min(255, int(maxColor * displayColor.g)) , 
         print >> output, min(255, int(maxColor * displayColor.b)) ,
      output.write('\n') # this is printed at the end of each line so that the
                         # next row goes on the next line
      sys.stdout.write('\rProgress: %s' % compute_progress_bar(j, height))
      sys.stdout.flush()
   sys.stdout.write('\rProgress: %s\n' % compute_progress_bar(1,1))
   sys.stdout.flush()

def print_header(width, height, output):
   print >> output, 'P3'
   print >> output, int(width), int(height)
   print >> output, maxColor

def compute_progress_bar(row, height):
   progress = row / float(height) * 50
   s = ['*'] * 50
   for p in range(int(progress)):
      s[p] = '#'
   percent = progress * 2
   return "".join(s) + ' ' +  str(int(percent)) + '%'
