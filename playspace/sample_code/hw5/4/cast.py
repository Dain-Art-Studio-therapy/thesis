...
def cast_ray(ray,sphere_list,amb_color,light,point):
   ...
   rscalar = near_sphere.finish.ambient*amb_color.r
   gscalar = near_sphere.finish.ambient*amb_color.g
   bscalar = near_sphere.finish.ambient*amb_color.b
   new_r = near_sphere.color.r*rscalar
   new_g = near_sphere.color.g*gscalar
   new_b = near_sphere.color.b*bscalar
...
def cast_all_rays(min_x,max_x,min_y,max_y,width,height,eye_point,sphere_list,Color,light,point):
   ...
   for j in range(height):
      for i in range(width):
         ...
         out.write(str(new_color.r*255) + ' ' + str(new_color.g*255)
                   + ' ' + str(new_color.b*255)+'\n')
