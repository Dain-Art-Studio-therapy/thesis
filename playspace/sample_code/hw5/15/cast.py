...
def specular_intensity(sphere, light, eye_pt, pe, N, Ldir):
   ...
   if spec_inten > 0:
      ...
      red = light.color.r * sphere.finish.specular * new_spec
      green = light.color.g * sphere.finish.specular * new_spec
      blue = light.color.b * sphere.finish.specular * new_spec
      return Color(red, green, blue)
   else:
      return Color(0.0, 0.0, 0.0)     
...
def cast_all_rays(min_x, max_x, min_y, max_y, width, height, eye_point, 
   ...
   with open('image.ppm', 'w') as f:
      print >>f, 'P3'
      print >>f, ''.join((str(width), ' ', str(height)))
      print >>f, '255'
      ...
      for y in range(height):
         for x in range(width):
            ...
            print >>f, ''.join((str(red), ' ', str(green), ' ', str(blue)))
