    ...
    if v <= []:
        ...
        for i in v:
            ...
            if (first > other):
                ...
                sh = i[0]
                firstp = i[1]
        n = collisions.sphere_at_normal(sh, firstp)
        length = vector_math.length_vector(norm)
        short = length * 0.01
        pe = vector_math.translate_point(firstp, length)
...
def cast_all_rays(min_x, max_x, min_y, max_y, width, height, eye_point,sphere_list, color, light):
    ...
    result = (cast_ray(ray, sphere_list, color))
    return (int(255 * result.r), int(255 * result.g), int(255 * result.b))
