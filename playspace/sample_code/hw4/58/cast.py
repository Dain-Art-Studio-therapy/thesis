   ...
   if closest_sphere is not None:
      ...
      if spec_intensity > 0:
         spec_val = data.Color(\
            ...
            spec_intensity**(1/intersected_sphere.finish.roughness))
      else: spec_val = data.Color(0, 0, 0)
...
def cast_all_rays(min_x, max_x, min_y, max_y, width, height,\
   ...
   for j in range(height): 
      for i in range(width):
         ...
         closest_sphere_color  = cast_ray(ray, sphere_list, color, light, eye_point)
         print min(int(closest_sphere_color.r*255), 255), \
...
def cast_all_rays_1(min_x, max_x, min_y, max_y, width, height, eye_point, sphere_list):
   dx = (max_x - min_x)/float(width)
   dy = (max_y - min_y)/float(height)
   for j in range(height): 
      for i in range(width):
         ray = data.Ray(eye_point, vector_math.vector_from_to(eye_point, data.Point(min_x + i*dx, max_y - j*dy, 0)))
