...
def lit_color(coltup,lcol,light,slist,oldc,eye):
    ...
    return specular(data.Color(oldc.r,oldc.g,oldc.b),lvec,vector_math.dot_vector(lvec,snorm),snorm,eye,pe,coltup,light)
        


def is_closest_collision(coltup,clist,light):
    for i in range(len(clist)):
        if utility.distance_3d(clist[i][1],light.pt) < utility.distance_3d(coltup[1],light.pt):
            return False
    return True
