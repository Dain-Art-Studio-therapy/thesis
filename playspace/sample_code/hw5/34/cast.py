...
def cast_ray(ray, sphere_list, ambient_color, light, eye_point):
    ...
    possible_list = []
    side_of_light = 0
    spec_light = data.Color(0.0,0.0,0.0)
    diffuse_light = data.Color(0.0,0.0,0.0) 
    if L != []:
        for i in L:
            ...
            cur_to_light_col = collisions.find_intersection_points(sphere_list, int_ray)
            ...
            if cur_to_light_col != []:
               ths_sph_to_lt_d = distance(pe, light.pt)  
               for index in cur_to_light_col:
                  ths_sph_to_oth_sph_d = distance(pe, index[1])
                  if ths_sph_to_oth_sph_d < ths_sph_to_lt_d:
...
def cast_all_rays(min_x, max_x, min_y, max_y, width, height, eye_point, sphere_list, ambient_color, light):
    ...
    for i in range(len(list_of_points)):
        ...
        bool = cast_ray(temp_ray, sphere_list, ambient_color, light, eye_point)
        ...
        print>>IMG, int(resulting_color.r), int(resulting_color.g), int(resulting_color.b)
    IMG.close()
