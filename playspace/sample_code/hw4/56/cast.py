...
def cast_ray_1(ray, sphere_list, ambient_light, light, eye_point):
   ...
   if intersections != None:
      closest_eye = closest_function(intersections, ray.pt)
      s1 = intersections[closest_eye][0]
      closest_eye_point = intersections[closest_eye][1]

      closest_pe = new_intersection(closest_eye_point, s1)
...
#      print specular_r, specular_g, specular_b
      ...
      if ldotn > 0:
         ...
         if light_intersections != None:
            ...
            for (s, p) in light_intersections:
               if vector_math.length_vector(vector_math.vector_from_to(p, light.pt)) < ldir_length:
                  shortest = vector_math.length_vector(vector_math.vector_from_to(p, light.pt))
                  shortest_sphere = p

            if shortest_sphere != s1:
                  r = s1.color.r * ambient_light.r * s1.finish.ambient + specular_r
                  g = s1.color.g * ambient_light.g * s1.finish.ambient + specular_g
                  b = s1.color.b * ambient_light.b * s1.finish.ambient + specular_b
...
def new_intersection(intersection, sphere):
   vector = collisions.sphere_normal_at_point(sphere, intersection)
   scale = vector_math.scale_vector(vector, .01)
   return vector_math.translate_point(intersection, scale)

def closest_function(intersections, point):
   closest = 0
   for i in range(1, len(intersections)):
      if vector_math.length_vector(vector_math.vector_from_to(point, intersections[i][1])) <= vector_math.length_vector(vector_math.vector_from_to(point, intersections[closest][1])):
         closest = i
