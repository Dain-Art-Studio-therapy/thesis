    ...
    if casts != []:
        ...
        for i in range(len(casts)):
            if casts[i][1] == nearest:
                ...
                finished = add_finish(used_sphere.color, used_sphere,
                 ambient_color)
                
                intense = add_specular_intensity(nearest, used_sphere, light, 
                eye_point, finished)
                
                if find_light_direction(nearest, used_sphere, light) > 0 and \
                not check_light_intersection(nearest, used_sphere, 
                light, sphere_list):
                    diffused = add_diffuse(nearest, used_sphere, light)
...
def add_specular_intensity(intersection, sphere, light, eye_point, color):
    ...
    eye_vector = vector_math.normalize_vector(
    ...
    if specular_intensity > 0:
        newr = light.color.r * sphere.finish.specular * \
        specular_intensity**(1/sphere.finish.roughness)
        
        newg = light.color.g * sphere.finish.specular * \
        specular_intensity**(1/sphere.finish.roughness)
        
        newb = light.color.b * sphere.finish.specular * \
        specular_intensity**(1/sphere.finish.roughness)
...
def check_light_intersection(point, sphere, light, sphere_list):
    off_point = compute_off_point(point, sphere)
    light_direction = vector_math.normalize_vector(
    vector_math.vector_from_to(compute_off_point(point, sphere), light.pt))
    #vector = vector_math.vector_from_to(normal, light_direction)
    ray = data.Ray(off_point, light_direction)
    ...
    for intersection in intersections:
        if euclidean_dist(off_point, light.pt) > euclidean_dist(off_point,
         intersection):
            return True
        else:
            return False
...
height, eye_point, sphere_list, ambient_color, light):
    ...
    pixels = []
    for ray in raylist:
       pixels.append(cast_ray(ray, sphere_list, ambient_color, light,
        eye_point))
    
    with utility.open_file("image.ppm", "w") as f:
        print >> f, "P3"
