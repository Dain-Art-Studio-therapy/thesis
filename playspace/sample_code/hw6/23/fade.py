import sys
import math
# argv = [fade.py, input.ppm, row, col, radius]
...
def get_new_grid(grid, argv):
   ...
   for y in range(height):
      ...
      for x in range(width):
         ...
         newpixel = [max(pixel[0] * scalar, pixel[0] * .2),
                     max(pixel[1] * scalar, pixel[1] * .2),
                     max(pixel[2] * scalar, pixel[2] * .2)]

         result[y].append(newpixel)
   return result
