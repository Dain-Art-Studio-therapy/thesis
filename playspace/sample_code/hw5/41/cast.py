...
def calc_specular_contribution(L_dir, N, L_dot_N, eye_pt, pe, refl_vect, V_dir):
...
def cast_ray(ray, sphere_list, amb, light, eye_pt):
    ...
    for c in cl:
        ...
        pe = vector_math.translate_point(nearest_pt, vector_math.scale_vector(N, 0.01))
        ...
        if spec_cont and difflight_cont2:
...
def cast_all_rays(min_x, max_x, min_y, max_y, width, height, eye_point, sphere_list, amb, light):
    ...
    for ray in rays:
        ...
        R = min(255, int(color.r * 255))
        G = min(255, int(color.g * 255))
        B = min(255, int(color.b * 255))
        print >> image_file, (R, G, B,)
