...
def obscure_check(main_point, point1, point2):
   if squared_distance(main_point, point1) > \
      squared_distance(main_point, point2):
      return True
   else:
      return False
def compare_distances_pt(input_list, ray):
   closest = 0
   for i in range(1, len(input_list)):
      nearest = squared_distance(ray.pt, input_list[closest][1])
      current = squared_distance(ray.pt, input_list[i][1])
      if current < nearest:
         closest = i
   return input_list[closest]
...
def normal_sphere_vector(sphere_intersect):
   if sphere_intersect != []:
      create_vector = vector_from_to(sphere_intersect[0].center,\
                      sphere_intersect[1])
...
def check_light_path(sphere_intersect, sphere_list, normalized_vector, pt_e, light):
   ...
   if visible_check <= 0:
      ...
      for sphere in sphere_list:
         ...
         if collision_test != []:
            intersect = compare_distances_pt(collision_test, test_ray) 
            sphere_pt = intersect[1]
            if obscure_check(light.pt, pt_e, sphere_pt) == True:
               return 0.0
      return sphere_intersect[0].finish.diffuse * visible_check

def check_specular_intensity(sphere_intersect, sphere_list, point, normalized_vector, pt_e, light):
   #normalized_vector = normalize_vector(vector_from_to(\
   #sphere_intersect[0].center,sphere_intersect[1]))
   #scaled_normal_v = scale_vector(normalized_vector, 0.01)
   #pt_e = translate_point(sphere_intersect[1], scaled_normal_v)
   ...
   reflection_vector = difference_vector(light_dir,scale_vector(normalized_vector,\
                       (2 * light_dot_norm)))
...
def color_convert(sphere, color, light, sphere_list, point, normalized_vector, pt_e): 
   if sphere != []:
      diffuse = check_light_path(sphere, sphere_list, normalized_vector, pt_e, light)
      specular = check_specular_intensity(sphere, sphere_list, point, normalized_vector, pt_e, light)
      red = min(int(((sphere[0].color.r * sphere[0].finish.ambient * color.r) + \
      (diffuse * sphere[0].color.r * light.color.r) + (specular * light.color.r)) * 255),255) 
      green = min(int(((sphere[0].color.g * sphere[0].finish.ambient * color.g) + \
      (diffuse * sphere[0].color.g * light.color.g) + (specular * light.color.g)) * 255),255)
      blue = min(int(((sphere[0].color.b * sphere[0].finish.ambient * color.b) + \
      (diffuse * sphere[0].color.b * light.color.b) + (specular * light.color.b)) * 255),255)
...
def cast_ray(ray, sphere_list, color, light, point):
   ...
   if intersect_test != []:
      ...
      return color_convert(closest_sphere, color, light, sphere_list,\
      point, normalized_vector, pt_e)
   else:
...
def cast_all_rays(min_x, max_x, min_y, max_y, width, height,\
   eye_point, sphere_list, color, light, image_file):
   width_increment = (max_x - min_x) / float(width)
   height_increment = (min_y - max_y) / float(height)
   open_image_file = open(image_file, 'w')
   print >> open_image_file, "P3"
   print >> open_image_file, int(width), int(height)
   print >> open_image_file, 255
   for height_point in iteration_floats(max_y,min_y,height_increment):
      for width_point in iteration_floats(min_x,max_x,width_increment):
         ...
         print >> open_image_file, colors[0], colors[1], colors[2]
   open_image_file.close()
