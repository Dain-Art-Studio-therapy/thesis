...
def cast_ray(ray, sphere_list, color, light, eye):
  ...
  if (len(hits) == 0):
    ...
    for i in range(1, len(hits)):
      if vm.length_vector(vm.difference_point(ray.pt, hits[i][1])) < vm.length_vector(vm.difference_point(ray.pt, hits[small][1])):
          small = i
...
def cast_all_rays(min_x, max_x, min_y, max_y, width, height, eye_point, sphere_list, color, light):
  ...
  while y > min_y:
    while x < max_x:
      ...
      print_scaled_pixel(cast_ray(rayThroughRec, sphere_list, color, light, eye_point))
...
def print_scaled_pixel(color):
  ...
  print int(color.red * max_pixel_val), ' ', int(color.green * max_pixel_val), ' ',int(color.blue * max_pixel_val), ' ',
