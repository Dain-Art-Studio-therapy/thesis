   ...
   for y in range(0,height):
      for x in range(0,width):
         ...
         if cast_ray(ray, sphere_list)== False:
            ...
            color_of_sphere(closest_sphere(ray, sphere_list, eye_point), color, ray, sphere_list, eye_point, light)
...
#Ambient light colors
   ...
   if how_bright > 0 and all_clear(sphere_list, point_e, light_vector) == True:
      if specular_intensity > 0: 
         print int(super_red), int(super_gre), int(super_blu)
      else:
         print int(red), int(gre), int(blu)
   else:
      print int(shade_r), int(shade_g), int(shade_b) 
